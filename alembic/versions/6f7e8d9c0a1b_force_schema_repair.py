...
def upgrade() -> None:
    ...
    if 'user_sessions' in existing_tables:
        ...
        if 'token' not in columns:
            op.add_column('user_sessions', sa.Column('token', sa.String(64), nullable=False))
        if 'user_id' not in columns:
            op.add_column('user_sessions', sa.Column('user_id', sa.Integer(), nullable=False))
        if 'created_at' not in columns:
            op.add_column('user_sessions', sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.func.now()))
    ...
    if 'user_settings' in existing_tables:
        ...
        if 'instructor_exclude' not in columns:
            op.add_column('user_settings', sa.Column('instructor_exclude', sa.JSON(), nullable=True))
        if 'title_exclude' not in columns:
            op.add_column('user_settings', sa.Column('title_exclude', sa.JSON(), nullable=True))
        if 'min_rating' not in columns:
            op.add_column('user_settings', sa.Column('min_rating', sa.Float(), nullable=False, server_default="0.0"))
        if 'course_update_threshold_months' not in columns:
            op.add_column('user_settings', sa.Column('course_update_threshold_months', sa.Integer(), nullable=False, server_default="24"))
        ...
        if 'created_at' not in columns:
            op.add_column('user_settings', sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.func.now()))
        if 'updated_at' not in columns:
            op.add_column('user_settings', sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.func.now()))

    # 4. enrollment_runs
    if 'enrollment_runs' in existing_tables:
        columns = [c['name'] for c in inspector.get_columns('enrollment_runs')]
        if 'status' not in columns:
            op.add_column('enrollment_runs', sa.Column('status', sa.String(length=50), nullable=False, server_default="pending"))
        if 'total_courses_found' not in columns:
            op.add_column('enrollment_runs', sa.Column('total_courses_found', sa.Integer(), nullable=False, server_default="0"))
        if 'total_processed' not in columns:
            op.add_column('enrollment_runs', sa.Column('total_processed', sa.Integer(), nullable=False, server_default="0"))
        if 'successfully_enrolled' not in columns:
            op.add_column('enrollment_runs', sa.Column('successfully_enrolled', sa.Integer(), nullable=False, server_default="0"))
        if 'already_enrolled' not in columns:
            op.add_column('enrollment_runs', sa.Column('already_enrolled', sa.Integer(), nullable=False, server_default="0"))
        if 'expired' not in columns:
            op.add_column('enrollment_runs', sa.Column('expired', sa.Integer(), nullable=False, server_default="0"))
        if 'excluded' not in columns:
            op.add_column('enrollment_runs', sa.Column('excluded', sa.Integer(), nullable=False, server_default="0"))
        if 'amount_saved' not in columns:
            op.add_column('enrollment_runs', sa.Column('amount_saved', sa.Float(), nullable=False, server_default="0.0"))
        if 'currency' not in columns:
            op.add_column('enrollment_runs', sa.Column('currency', sa.String(length=10), nullable=False, server_default="usd"))
        if 'error_message' not in columns:
            op.add_column('enrollment_runs', sa.Column('error_message', sa.Text(), nullable=True))
        if 'started_at' not in columns:
            op.add_column('enrollment_runs', sa.Column('started_at', sa.DateTime(), nullable=False, server_default=sa.func.now()))
        if 'completed_at' not in columns:
            op.add_column('enrollment_runs', sa.Column('completed_at', sa.DateTime(), nullable=True))
    ...
    if 'enrolled_courses' in existing_tables:
        ...
        if 'slug' not in columns:
            op.add_column('enrolled_courses', sa.Column('slug', sa.String(length=255), nullable=True))
        if 'course_id' not in columns:
            op.add_column('enrolled_courses', sa.Column('course_id', sa.String(length=50), nullable=True))
        if 'coupon_code' not in columns:
            op.add_column('enrolled_courses', sa.Column('coupon_code', sa.String(length=100), nullable=True))
        if 'price' not in columns:
            op.add_column('enrolled_courses', sa.Column('price', sa.Float(), nullable=True))
        if 'category' not in columns:
            op.add_column('enrolled_courses', sa.Column('category', sa.String(length=100), nullable=True))
        if 'language' not in columns:
            op.add_column('enrolled_courses', sa.Column('language', sa.String(length=50), nullable=True))
        if 'rating' not in columns:
            op.add_column('enrolled_courses', sa.Column('rating', sa.Float(), nullable=True))
        if 'site_source' not in columns:
            op.add_column('enrolled_courses', sa.Column('site_source', sa.String(length=100), nullable=True))
        if 'status' not in columns:
            op.add_column('enrolled_courses', sa.Column('status', sa.String(length=50), nullable=False, server_default="enrolled"))
        if 'error_message' not in columns:
            op.add_column('enrolled_courses', sa.Column('error_message', sa.Text(), nullable=True))
        if 'enrolled_at' not in columns:
            op.add_column('enrolled_courses', sa.Column('enrolled_at', sa.DateTime(), nullable=False, server_default=sa.func.now()))
