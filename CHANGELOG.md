...
- **Dedicated Above-the-Fold `/login` Page & Navigation Links ([`app/routers/dashboard.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/routers/dashboard.py), [`app/templates/pages/login_page.html`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/templates/pages/login_page.html), [`app/templates/components/base.html`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/templates/components/base.html))**:
  - Implemented dedicated GET `/login` and `/login/` route handlers rendering `pages/login_page.html` with an uncluttered card layout positioned directly above the fold, replacing the deep 303 redirect to homepage `/#connect`.
  - Added desktop header action button (`#login-nav-btn`, responsive `hidden sm:inline-flex`) and mobile drawer navigation link (`#mobile-drawer-login-link`, 44px WCAG touch target) with reactive client-side DOM auth state toggling.
  - Implemented anonymous double-submit CSRF cookie issuance (`csrf_token` / `__Host-csrf_token`) on `/login` render with unconditional cleanup of stale tokens with conflicting prefixes.
  - Added authenticated session bypass automatically redirecting active sessions to `/dashboard` (HTTP 303).
  - Added dedicated `/login` unit test assertions in [`tests/test_login_page.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/tests/test_login_page.py).
...
### Fixed
- **Form Security Hardening & CWE-598 Prevention ([`app/templates/pages/login.html`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/templates/pages/login.html), [`app/templates/pages/login_page.html`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/templates/pages/login_page.html))**:
  - Bound explicit `method="POST"` and API actions (`/api/auth/login` and `/api/auth/login/cookies`) to login forms, preventing browser fallbacks from defaulting to HTTP GET and leaking session credentials in URL query strings if JavaScript fails.
  - Updated client-side CSRF extraction regex to explicitly prioritize `__Host-csrf_token` over `csrf_token`, preventing dual-cookie token mismatch 403 errors.
  - Normalized cache-control path evaluation in [`main.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/main.py) with `path.rstrip("/")` to ensure trailing-slash routes inherit `no-cache, no-store` directives.
...
- **CLI and Desktop GUI Fresh-System Regression Tests ([`tests/test_gui_bridge.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/tests/test_gui_bridge.py), [`tests/test_build_exe.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/tests/test_build_exe.py), [`tests/test_cli.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/tests/test_cli.py))**: Added 6 regression tests covering `_handle_fetch_stats` `successfully_enrolled` calculation across populated and empty states, polling teardown exception absorption, `cli.spec` root module `"main"` hidden import, `upx=False` packaging anti-heuristic assertions, and end-to-end fresh SQLite database and table auto-creation via isolated subprocess.
...
- **README Usability Streamlining & Multi-Persona Architecture ([`README.md`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/README.md))**: Refactored `README.md` from 726 lines down to 348 lines (~52% reduction), organizing into an 8-section user-centric layout serving 4 distinct user personas: Casual users (Option 1: standalone `Gui.exe`/`Gui` binary releases link), Python GUI users (Option 2: 2-command CustomTkinter setup with Linux `python3-tk` notes), Homelab/Server users (Option 3: Docker Compose with stdlib `secrets`/`base64` inline keygen), and Power/CLI users (Section 5: 8-row Typer/Rich command cheatsheet and execution flags). Consolidated 5 scattered disclaimers into 1 Hero Callout and Section 8 Responsible Use block, highlighted 1-click safe browser cookie extraction with a collapsible `<details>` F12 fallback, purged maintainer deployment checklists and keyword spam, and strictly preserved the Alembic migration test invariance contract (`scripts/alembic_upgrade_pinned.py`).

### Fixed
- **Desktop GUI History Run Attribute Fix ([`app/gui/bridge.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/gui/bridge.py))**: Fixed `AttributeError` by referencing `r.successfully_enrolled` instead of non-existent `r.enrolled_count` on `EnrollmentRun` in `_handle_fetch_stats`.
- **Fresh-System SQLite Auto-Initialization ([`app/cli/main.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/cli/main.py), [`app/gui/app.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/gui/app.py), [`gui.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/gui.py))**: Auto-initialized schema via `create_tables()` during CLI subcommands and GUI launch, preventing `sqlite3.OperationalError: no such table` on fresh systems with zero database.
- **Standalone Server Module Closure & UPX Hardening ([`cli.spec`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/cli.spec), [`gui.spec`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/gui.spec))**: Added root `"main"` to `cli.spec` hiddenimports for `cli.exe server` and set `upx=False` in both specs to prevent Windows Defender heuristic false positives.
- **GUI Teardown Polling Guard ([`app/gui/app.py`](file:///run/media/madhud/Storage1/LinuxProjects/Codes/Projects/Udemy%20Enroller/app/gui/app.py))**: Wrapped `self.after` in an exception guard to silently absorb teardown `_tkinter.TclError` when closing the window.
